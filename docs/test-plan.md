...
- Scripted hotkey smoke: launch the app and fixture window, synthesize `Cmd+Shift+2`, drag a known region, and verify `capture_ocr_finished` plus clipboard text. Startup policy tests must also lock the `Cmd+Shift+0` fallback for default registration failure and preserve an explicitly user-selected `Cmd+Shift+0`.
...
- Settings engine availability: verify Vision is the default only on macOS/Vision-capable builds; unsupported platforms must show it disabled or normalize the default/persisted `vision` settings back to PaddleOCR. Explicit `paddleocr` settings must remain preserved.
...
- Permission guidance smoke: compile the real permission helper panel content and verify it exposes the draggable app icon, a large left-pointing direction cue aligned with the instruction text, minimal copy that names the left Screen Recording list destination, and a non-key/non-main helper panel that cannot steal keyboard focus from System Settings authentication prompts. Settings layout smoke must also verify that the app can programmatically focus the Capture permission page when permission is missing.
...
- Default OCR engine changes should be backed by a representative real-screen corpus with exact expected text, per-engine CER/latency reports, and no material regression on Korean, English, code-like, dense, and wide-strip cases. A user-directed default change may ship with narrower smoke evidence if the residual quality risk is documented and PaddleOCR remains selectable.
...
- Permission guidance changes must preserve a clean icon-plus-left-arrow instruction, align the arrow and text in one row, name the left Screen Recording list destination, avoid extra destination cards/explanatory copy, keep the helper unable to become key/main, and keep permission-missing Settings entry focused on the Capture page.
